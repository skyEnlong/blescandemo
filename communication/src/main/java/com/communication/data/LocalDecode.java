...
public class LocalDecode {
			...
			if (length == 6) {
				...
				case SLEEPDATA_STATE:
					if (isTodaySleep(curTime)) {
						...
						if (lastTime / 600000 + 1 <= curTime / 600000) {
							...
							Log.e(TAG, "start_time has calculated");

							Log.i(TAG,
									"last sleep start_time is:"
											+ mFormat
	...
	/**
	 * check start_time is curDay or not
	 * 
	 * @param curTime2
	 * @return
	 */
	...
	/**
	 * check start_time is curSleepday or not
	 * 
	 * @param curTime2
	 * @return
	 */
...
//		Log.d(TAG, "base start_time:" + src + " curtime:" + des + " isTodaySleep"
//				+ isSleepToday);
		return isSleepToday;
	}
	...
	protected void filterSportResult(int[] result) {
		...
		if (null != deviceName && deviceName.equals("CANDY")) { //
	...
	protected long getSystemTime() {
		...
		Log.d("debug",
				"system start_time is " + cal.get(Calendar.YEAR) + "-"
	...
	protected long createTime(ArrayList<Integer> list, boolean isEnd) {
		...
		Log.d("debug", "start_time is " + year + "-" + month + "-" + day + "-" + hour
				+ "-" + minute);
	...
	/**
	 * check start_time isInvalid <0 true
	 * 
	 * @param time
	 * @return
	 */
