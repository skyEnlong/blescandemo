...
public class SleepDataUtil {
    ...
    /**
     * delete all data by entime
     * @param user_id
     * @param end_time
     */
    public void deleteDataByTime(String user_id, long end_time){
        mSleepDetailDB.open();
        mSleepDetailDB.deleteByUserId(user_id, end_time);
        mSleepDetailDB.close();
    }
...
//			saveInfo2File(detail);
    ...
    public List<SleepDetailTB> getSleepDetailList(String userid, long start, long end){
	...
	public int updateSleepDetail(String userID, long start, long end, int[] detail){
		...
		if(null != detail){
			...
			for(int i= 0; i< detail.length; i++){
				...
				tb.sleepvalue = ((detail[i] & 0xff) > 0xff) ? -1 : (detail[i] & 0xff) ;
...
//			String fileName = "qqhealth-sleep" + start_time + "-" + timestamp + ".log";
