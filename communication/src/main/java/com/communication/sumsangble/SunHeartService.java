...
public class SunHeartService extends Service {
	...
	private int battery;
	private boolean isKillBySys = true;
	...
	public static final String ACTION_KILLED = "com.communication.sumsangble.KILL";
	
	public static final String SEARTCH_STOP = "com.communication.sumsangble.stop_seartch";
	...
	public void onCreate() {
		...
		mHeartManager = new SunHeartBLEGatt(this.getApplicationContext(), new BLEConnectCallback() {
			...
			public boolean onConnectStateChanged(BluetoothDevice device,
				...
				if (newState == BluetoothAdapter.STATE_CONNECTED) {
					...
					if (isStart && status != BluetoothGatt.GATT_SUCCESS) {
						...
						tryCount = DEFAULT_COUNT << 3;
						reConnect();
					}
				}

				CLog.i(TAG, "connect state:" + isConnect);
				return false;
			}
			...
			@Override
			public boolean onSeartchCodoonCallBack(CodoonBluethoothDevice device) {
				// TODO Auto-generated method stub
				return false;
			}
		});
		...
		mDisconveryManager =  new DisconveryManager(this , new OnSeartchCallback() {
...
//			@Override
//			public boolean onSeartch(BluetoothDevice device, byte[] otherInfo) {
//				return false;
//			}

			@Override
			public boolean onSeartch(CodoonBluethoothDevice c_device,
					byte[] otherInfo) {
				// TODO Auto-generated method stub

				String name = c_device.getName();
				BluetoothDevice device = c_device.getDevice();
				CLog.i(TAG, "find device: " + name);
				 
				for (BLEConnectCallback mIConnectCallback : mIConnectCallbacks) {
					mIConnectCallback.onSeartchCodoonCallBack(c_device);
				}
		...
		mHandler = new Handler() {
			...
			public void handleMessage(Message msg) {
				...
				case CONECT_HEART:
					...
					mDevice = mBluetoothAdapter.getRemoteDevice(mDevice.getAddress());
					CLog.d(TAG, "connect heart sensor:" + mDevice.getName());
					mHeartManager.connect(mDevice, MobileUtil.isAutoConnect());
					break;
				case SEARTCH_TIMEOUT:
					CLog.i(TAG, "seartch start_time out");
					stopScan();
					for (BLEConnectCallback mIConnectCallback : mIConnectCallbacks) {
						mIConnectCallback.onSeartchCodoonCallBack(null);
					}

					break;
				case CONNECT_TIMEOUT:
					CLog.i(TAG, "connect start_time out");
					if (tryCount-- > 0) {
	...
	public void onDestroy() {
		...
		if(isKillBySys){
			sendBroadcast(new Intent(ACTION_KILLED));
		}
	...
	public void stopScan() {
		sendBroadcast(new Intent(SEARTCH_STOP));
		isSeartching = false;
		mHandler.removeMessages(ACTION_SEARTCH);
		mHandler.removeMessages(SEARTCH_TIMEOUT);
		mDisconveryManager.stopSearch();
	...
	public class LocalBinder extends Binder {
		...
		public void registerConnectCallBack(BLEConnectCallback mIConnectCallback) {
			if(null != mIConnectCallback && !mIConnectCallbacks.contains(mIConnectCallback)){
